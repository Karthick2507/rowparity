...
def _run(args) -> int:
    ...
    if args.result_sink:
        try:
            result_sink = make_result_sink(
                args.result_sink, run_id, table_prefix=args.result_sink_prefix
            )
...
def _report(args) -> int:
    try:
        html = render_report_from_sink(
            args.result_sink, table_prefix=args.result_sink_prefix, days=args.days
        )
    except Exception as exc:
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        prog="rowparity", description="Fingerprint-based data comparison for CI."
    )
    ...
    report_p.add_argument(
        ...
        default="rowparity",
        metavar="PREFIX",
        help="must match the prefix used when writing (default: rowparity)",
    )
    report_p.add_argument(
        "--days", type=int, default=21, metavar="N", help="history window (default: 21)"
    )
    report_p.add_argument(
        "--html", required=True, metavar="FILE", help="write the HTML report here"
    )
