...
def _r(**kwargs):
    defaults = dict(equivalent=True, keys=["id"], compared_columns=["id", "v"],
                     expected_rows=100, actual_rows=100)
...
def test_read_raw_history_respects_window(db_path):
    _write_days(db_path, "case_a", [], [
        (0, _r()), (1, _r()), (40, _r()),  # 40 days out -> outside a 21-day window
    ])
...
def test_build_case_histories_one_point_per_day_latest_wins(db_path):
    ...
    sink.write("case_a", [], _r(equivalent=False, changed_count=5), run_ts=BASE + timedelta(hours=6))
    sink.close()
...
def test_build_case_histories_change_signatures_grouped():
    ...
    result.examples = [
        RowDiff(kind="changed", key=(("i", 1),), columns=[ColumnDiff(column="v", expected=1, actual=2)]),
        RowDiff(kind="changed", key=(("i", 2),), columns=[ColumnDiff(column="v", expected=3, actual=4)]),
    ]
    summary_rows = [{
        "run_id": "r1", "run_ts": BASE, "case_name": "case_a", "tags": "[]",
        "equivalent": False, "expected_rows": 100, "actual_rows": 100,
        "missing_count": 0, "added_count": 0, "changed_count": 2,
        "compared_columns": "[]", "keys": '["id"]', "type_mismatches": "[]",
        "columns_only_in_expected": "[]", "columns_only_in_actual": "[]",
    }]
...
def test_render_html_report_contains_case_data_and_valid_json():
    cases = [{
        "name": "demo_case", "tags": ["t"], "keys": ["id"],
        "history": [
            {"date": "2026-07-01", "run_id": "r1", "equivalent": True,
             "expected_rows": 10, "actual_rows": 10, "missing_count": 0, "added_count": 0, "changed_count": 0,
             "schema": {"onlyExpected": [], "onlyActual": [], "typeMismatches": []}, "schema_drifted": False},
        ],
        "signatures": [], "diffs": [],
    }]
    html = render_html_report(cases, days=21, generated_at=datetime(2026, 7, 21, 12, 0, tzinfo=timezone.utc))
...
def test_render_html_report_escapes_script_close_in_data():
    ...
    cases = [{
        "name": "demo_case", "tags": [], "keys": ["id"],
        "history": [{
            "date": "2026-07-01", "run_id": "r1", "equivalent": False,
            "expected_rows": 1, "actual_rows": 1, "missing_count": 0, "added_count": 0, "changed_count": 1,
            "schema": {"onlyExpected": [], "onlyActual": [], "typeMismatches": []}, "schema_drifted": False,
        }],
        "signatures": [{"columns": ["v"], "count": 1, "example": "key=(1): v: a -> </script><script>alert(1)"}],
        "diffs": [{"key": "(1)", "column": "v", "oldv": "a", "newv": "</script><script>alert(1)"}],
    }]
    html = render_html_report(cases, days=21)
    assert "</script><script>alert(1)" not in html
    assert "<\\/script><script>alert(1)" in html  # escaped, safe as a JS string literal
    assert html.count("</script>") == 1  # only the report's own closing tag -- nothing closed it early


def test_cli_report_command_end_to_end(db_path, tmp_path, capsys):
    _write_days(db_path, "case_a", ["demo"], [(0, _r()), (1, _r(equivalent=False, changed_count=1))])
    out_html = str(tmp_path / "out.html")

    rc = cli_main(["report", "--result-sink", f"duckdb:{db_path}", "--html", out_html, "--days", "30"])
    assert rc == 0
